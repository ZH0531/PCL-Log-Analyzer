# PCL Log Analyzer - Error Recognition Rules
# 错误识别规则配置文件
...
function Get-ErrorTypes {
    return @(
        # ===== 根本原因（最高优先级，优先解决） =====
        @{ Pattern = '/ERROR\].*Missing.*dependencies|/ERROR\].*unsupported mandatory'; Type = 'Mod依赖缺失'; Severity = '严重'; Priority = 1 },
        @{ Pattern = '/FATAL\].*requires .+ or above|/ERROR\].*requires .+ or above'; Type = 'Mod版本不匹配'; Severity = '严重'; Priority = 2 },
        @{ Pattern = '/ERROR\].*ModResolutionException'; Type = 'Mod依赖问题'; Severity = '严重'; Priority = 3 },
        @{ Pattern = '/ERROR\].*Failed to parse.*(config|json|data)|/ERROR\].*(Not a string|Not a json array)|MalformedJsonException'; Type = '配置文件格式错误'; Severity = '中等'; Priority = 4 },
        
        # ===== 直接错误（导致崩溃的具体问题） =====
        @{ Pattern = '/FATAL\].*Error during.*loading|/ERROR\].*Failed to create mod instance'; Type = 'Mod加载失败'; Severity = '严重'; Priority = 10 },
        @{ Pattern = '/ERROR\].*(Caught exception during event FMLClientSetupEvent|Exception caught during firing event)'; Type = 'Mod初始化失败'; Severity = '严重'; Priority = 11 },
        ...
        @{ Pattern = '/FATAL\]'; Type = '致命错误'; Severity = '严重'; Priority = 21 },
        
        # ===== 连带错误（解决前面的问题后会自动消失） =====
        @{ Pattern = '/ERROR\].*Cowardly refusing to send event.*to a broken mod state'; Type = 'Mod状态异常'; Severity = '严重'; Priority = 30 },
        @{ Pattern = '/ERROR\].*Skipping.*due to previous error'; Type = 'Mod跳过加载'; Severity = '严重'; Priority = 31 },
        ...
        @{ Pattern = '/ERROR\].*Failed to load model|/ERROR\].*ModelManager'; Type = '模型加载失败'; Severity = '中等'; Priority = 40 },
        @{ Pattern = '/ERROR\].*Invalid path in pack'; Type = '资源路径错误'; Severity = '中等'; Priority = 41 },
        @{ Pattern = '/ERROR\].*was null.*due to some mod not registering'; Type = '资源注册缺失'; Severity = '中等'; Priority = 42 },
